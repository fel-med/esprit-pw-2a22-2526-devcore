(function () {
  ...
  const STORAGE_KEY = "cre8_event_center_stats_visible";
  ...
  function setupStatsToggle() {
    document.querySelectorAll("[data-ec-stats]").forEach((panel) => {
      applyStatsState(panel);
      const button = panel.querySelector("[data-ec-stats-toggle]");
      if (!button || button.dataset.ecBound === "1") return;
      button.dataset.ecBound = "1";
  ...
  function replaceResultsFromUrl(url, push) {
    ...
    fetch(url, { headers: { "X-Requested-With": "XMLHttpRequest" }, credentials: "same-origin" })
      ...
      .then((html) => {
        ...
        const nextRegion = doc.getElementById("ecResultsRegion");
