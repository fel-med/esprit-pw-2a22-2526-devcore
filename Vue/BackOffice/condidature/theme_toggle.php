<button type="button" class="theme-toggle theme-toggle-admin" data-theme-toggle data-default-theme="dark" aria-label="Switch to light mode">
...
(() => {
    ...
    function applyTheme(theme, fallback = 'dark') {
        const normalized = normalizeTheme(theme, fallback);
        document.documentElement.setAttribute('data-theme', normalized);
        document.querySelectorAll('[data-theme-toggle]').forEach((button) => {
            const label = button.querySelector('[data-theme-toggle-label]');
            const icon = button.querySelector('[data-theme-toggle-icon]');
            const meta = themeMeta[normalized] || themeMeta[fallback] || themeMeta.dark;
            if (label) {
    ...
    const defaultTheme = normalizeTheme(button ? button.dataset.defaultTheme : '', 'dark');
