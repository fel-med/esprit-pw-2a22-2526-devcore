...
if (isset($_GET['export_csv'])) {
    ...
    fputcsv($out, ['ID', 'Name', 'Description', 'Characteristics', 'Price (' . DEVISE . ')', 'Image', 'Brand ID']);
    foreach ($liste as $p) {
        fputcsv($out, [
            $p['idProduit'],
            $p['nomProduit'],
...
<head>
    ...
    <style>
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .upload-preview-name { font-size:11px; color:var(--text-muted); }
        ...
        .td-desc { color: var(--text-muted); max-width: 160px; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }
        .td-carac { color: var(--text-dim); font-size: 12px; max-width: 120px; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }
        ...
        .td-img-thumb img { width: 42px; height: 42px; object-fit: cover; border-radius: var(--radius-sm); border: 1px solid var(--border); cursor: pointer; transition: opacity .15s, border-color .15s; display: block; }
...
<aside class="sidebar">
    ...
    <nav class="sidebar-nav">
        ...
        <a class="nav-item" href="#">
            <svg class="nav-icon" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.8"><path stroke-linecap="round" stroke-linejoin="round" d="M7 12l3-3 3 3 4-4M8 21l4-4 4 4M3 4h18M4 4h16v12a1 1 0 01-1 1H5a1 1 0 01-1-1V4z"/></svg>
            Offers & Applications
        </a>
        <a class="nav-item" href="#">
        ...
        <a class="nav-item" href="#">
            <svg class="nav-icon" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.8"><path stroke-linecap="round" stroke-linejoin="round" d="M13 10V3L4 14h7v7l9-11h-7z"/></svg>
            Campaigns & Contracts
        </a>
        <a class="nav-item active" href="index.php">
            <svg class="nav-icon" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.8"><path stroke-linecap="round" stroke-linejoin="round" d="M20 7l-8-4-8 4m16 0l-8 4m8-4v10l-8 4m0-10L4 7m8 10V11"/></svg>
            Products
        </a>
        <a class="nav-item" href="#">
            <svg class="nav-icon" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.8"><path stroke-linecap="round" stroke-linejoin="round" d="M7 8h10M7 12h4m1 8l-4-4H5a2 2 0 01-2-2V6a2 2 0 012-2h14a2 2 0 012 2v8a2 2 0 01-2 2h-3l-4 4z"/></svg>
            Posts & Comments
        </a>
    </nav>
...
<div class="topbar">
    ...
    <div class="topbar-actions">
        <!-- THEME TOGGLE -->
        <button class="theme-toggle" id="themeToggleBtn" onclick="toggleTheme()" title="Switch theme" type="button">
            <!-- Sun icon -->
            ...
            <div class="toggle-track-pill" id="togglePill">
        ...
        <div class="search-wrap">
            ...
            <input type="text" class="search-input" placeholder="Search a product…" id="searchInput" oninput="filterTable()">
        </div>
...
<div class="content">
    ...
    <div class="kpi-strip">
        ...
        <div class="kpi-card kpi-danger">
            <div class="kpi-label">No Image</div>
            <div class="kpi-value"><?= $sanImage ?></div>
            <div class="kpi-sub"><?= $totalProduits > 0 ? round($sanImage / $totalProduits * 100) : 0 ?>% of catalog</div>
        </div>
    </div>
    ...
    <div class="img-alert-banner">
        ...
        <span><strong><?= $sanImage ?> product<?= $sanImage > 1 ? 's' : '' ?></strong> without an image in the catalog — this may hurt presentation on the creator side.</span>
        <button class="img-alert-btn" onclick="setImageFilter('without')">Filter</button>
    </div>
    <?php endif; ?>
    ...
    <div class="form-panel">
        ...
        <div class="form-body">
            <form method="POST" action="index.php" enctype="multipart/form-data" id="produitForm">
                ...
                <div class="form-grid">
                    <div class="form-group">
                        ...
                        <input type="text" name="nom" class="form-control" placeholder="e.g. Leather handbag"
                    ...
                    <div class="form-group">
                        ...
                        <input type="number" step="0.01" min="0" name="prix" class="form-control"
                               placeholder="0.00" value="<?= htmlspecialchars($produitUpdate['prix'] ?? '') ?>" required>
                    </div>
                    <div class="form-group form-col-full">
                        <label class="form-label">Description</label>
                        <textarea name="description" class="form-control"
                                  placeholder="Detailed product description…"><?= htmlspecialchars($produitUpdate['description'] ?? '') ?></textarea>
                    </div>
                    <div class="form-group form-col-full">
                        <label class="form-label">Characteristics</label>
                        <textarea name="caracteristiques" class="form-control" style="min-height:64px"
                                  placeholder="Color, size, material, specifications…"><?= htmlspecialchars($produitUpdate['caracteristiques'] ?? '') ?></textarea>
                    </div>
                    <div class="form-group form-col-full">
                    ...
                    <div class="form-group form-col-full">
                        <div class="form-todo-note">
                            ...
                            <span><strong>TODO dev:</strong> idMarque is currently hardcoded to <code style="background:rgba(245,158,11,.15);padding:1px 5px;border-radius:3px">1</code>. After merging the user module, replace with <code style="background:rgba(245,158,11,.15);padding:1px 5px;border-radius:3px">$_SESSION['userId']</code> to auto-link the product to the logged-in brand.</span>
                        </div>
                    </div>
                    <?php endif; ?>
                </div>
    ...
    <div class="filter-bar">
        ...
        <div class="filter-group">
            ...
            <div class="image-filter-row">
                <button class="chip active" id="chip-all"     onclick="setImageFilter('all')">All</button>
                <button class="chip"        id="chip-with"    onclick="setImageFilter('with')">With image</button>
                <button class="chip"        id="chip-without" onclick="setImageFilter('without')">No image</button>
            </div>
        </div>
        ...
        <table id="produitsTable">
            <thead>
                <tr>
                    ...
                    <th class="no-sort">Characteristics</th>
                    <th class="no-sort">Brand</th>
                    <th onclick="sortTable('prix')" data-col="prix">Price (<?= DEVISE ?>)</th>
                ...
                <tr data-id="<?= $p['idProduit'] ?>"
                        ...
                        <?php if (!empty($p['image'])): ?>
                            <div class="td-img-thumb">
                                <img src="<?= $baseUrl ?>/Vue/public/produits/<?= htmlspecialchars($p['image']) ?>"
                                     alt="" onclick="openPreview(<?= $p['idProduit'] ?>)" title="Quick preview">
                    ...
                    <td>
                        <?php if (!empty($p['nomMarque'])): ?>
                            <span class="td-marque" title="<?= htmlspecialchars($p['nomMarque']) ?>"><?= htmlspecialchars($p['nomMarque']) ?></span>
                    ...
                    <td>
                        <span class="prix-badge" id="prixDisplay-<?= $p['idProduit'] ?>"
                              style="cursor:pointer" title="Click to edit price"
                              onclick="showInlineEdit(<?= $p['idProduit'] ?>, <?= $p['prix'] ?>)">
                            <?= number_format((float)$p['prix'], 2) ?> <?= DEVISE ?>
                        </span>
                        <form method="POST" action="index.php" class="prix-inline-form"
                              id="prixForm-<?= $p['idProduit'] ?>" style="display:none">
                            ...
                            <input type="number" name="prix" step="0.01" min="0"
                                   class="prix-inline-input" id="prixInput-<?= $p['idProduit'] ?>"
                                   value="<?= $p['prix'] ?>">
                            <button type="submit" class="btn-inline-save" title="Save">✓</button>
                            <button type="button" class="btn-inline-cancel" title="Cancel"
                                    onclick="hideInlineEdit(<?= $p['idProduit'] ?>)">✕</button>
                        </form>
                    </td>
                    <td>
                        <div class="action-group">
                            <button type="button" class="btn-action btn-view"
                                    onclick="openPreview(<?= $p['idProduit'] ?>)" title="Admin preview">
                                <svg width="11" height="11" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"/><path stroke-linecap="round" stroke-linejoin="round" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"/></svg>
                                View
                            </button>
                            <a href="<?= $baseUrl ?>/Vue/FrontOffice/produit/indexC.php?voir=<?= $p['idProduit'] ?>"
                               target="_blank" class="btn-action btn-fo" title="View on creator side">
...
<div class="modal-overlay" id="previewModal" onclick="closePreviewOutside(event)">
    <div class="preview-modal" id="previewBox">
        ...
        <div class="preview-modal-body">
            ...
            <div id="previewMarqueBadge" style="margin-bottom:12px"></div>
            <div class="preview-section-label">Description</div>
            <div id="previewDesc" class="preview-text"></div>
            <div class="preview-section-label">Characteristics</div>
...
// PAGINATION
// ══════════════════════════════════════════
function renderPage(page, q, pMin, pMax) {
    currentPage = page;
...
function openPreview(id) {
    ...
    if (p.marque) {
        marqueBadge.innerHTML = `<span class="preview-marque-badge">
    ...
    document.getElementById('previewFoBtn').href =
        BASE_URL + '/Vue/FrontOffice/produit/indexC.php?voir=' + p.id;
